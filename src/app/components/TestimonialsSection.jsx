...
const fallbackTestimonials = [
  {
    personName: "Adarsh Shinde",
    city: "Shirdi",
    content:
      "A beautiful escape and one of the best resorts near Shirdi. The calm surroundings, warm hospitality and clean spaces made our stay truly special.",
  },
  {
    personName: "Ranjeet Gaikwad",
    city: "Pune",
    content:
  ...
  {
    personName: "Pooja Patil",
    city: "Pune",
    content:
      "One of the best resorts near Shirdi for a peaceful family stay. The greenery, comfort and warm service made everything feel effortless.",
  },
  {
    personName: "Nikhil Sharma",
    city: "Hyderabad",
    content:
      "If you are searching for the best villas and cottages with calm surroundings, this place is exactly what you need. It feels private, clean and relaxing.",
  },
  {
...
function TestimonialCard({ testimonial }) {
  ...
  return (
    <article
      ...
      className="h-full rounded-[26px] border border-[#f1eadc] bg-[#fffdf8] p-6 shadow-[0_18px_50px_rgba(36,31,21,0.08)] sm:rounded-[30px] sm:p-8"
    >
      ...
      <div className="mt-6 border-t border-[#ebe6d9] pt-4 sm:mt-7 sm:pt-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[#7d8579]">{testimonial.city}</p>
          </div>
        </div>
      </div>
    </article>
...
export default function TestimonialsSection() {
  ...
  useEffect(() => {
    ...
    const loadTestimonials = async () => {
      try {
        ...
        const records = Array.isArray(payload.testimonials) ? payload.testimonials : [];
  ...
  useEffect(() => {
    ...
    if (!track || testimonials.length < 2) {
      return undefined;
    }
        ...
        <div className="mt-10">
          ...
          >
            {cards.map((testimonial) => (
              <div
                key={`${testimonial.personName}-${testimonial.city}-${testimonial.content.slice(0, 16)}`}
                className="w-[86vw] flex-none snap-center sm:w-[400px] lg:w-[440px]"
              >
