...
const dashboardCards = [
  { label: "Total Bookings", value: "128", note: "12 new this week", icon: CalendarDays },
  { label: "Contact Enquiries", value: "34", note: "5 waiting for reply", icon: MessageSquareText },
  { label: "Website Leads", value: "42", note: "8 pending follow-ups", icon: Users },
  { label: "System Health", value: "Good", note: "Protected and active", icon: CheckCircle2 },
];
...
export default async function AdminDashboardPage() {
  ...
  return (
    <ConsoleShell pageTitle="Dashboard" pageDescription="Live business tracking across bookings, enquiries and content management.">
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {dashboardCards.map((card) => <DashboardStat key={card.label} card={card} />)}
      </section>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_0.95fr]">
        <article className="rounded-[28px] border border-[#e7e2d3] bg-white p-6 shadow-[0_16px_45px_rgba(36,48,38,0.08)] sm:p-8">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#6b8444]">Quick Actions</p>
              <h3 className="mt-2 font-heading text-[clamp(1.8rem,2.6vw,2.8rem)] text-[#18352a]">Dashboard overview</h3>
            </div>
            <BarChart3 className="h-6 w-6 text-[#6b8444]" />
          </div>
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {["Review new enquiries", "Check booking requests", "Publish blogs and banners", "Manage lead follow-ups"].map((item) => (
        ...
        <article className="rounded-[28px] border border-[#e7e2d3] bg-white p-6 shadow-[0_16px_45px_rgba(36,48,38,0.08)] sm:p-8">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-heading text-[clamp(1.7rem,2.4vw,2.4rem)] text-[#18352a]">Recent Activity</h3>
          </div>
          <div className="mt-5 space-y-4">
            {activityItems.map((item, index) => (
              <div key={item} className="flex items-center gap-3 rounded-[20px] bg-[#fbf8ef] p-4">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#eef4e3] text-[#6b8444]">{index + 1}</span>
